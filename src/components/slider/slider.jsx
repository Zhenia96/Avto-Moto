...
function Slider({ className }) {
  ...
  return (
    <div className={`${className ?? ''} slider`}>
      ...
      <img
        className='slider__main-picture'
        src={currentPicture.url.main}
        alt={currentPicture.description}
        width='600'
        height='375'
      />
      <button className='slider__button slider__button--back' disabled={isBackButtonDisabled(currentPictureIndex, pictures)} onClick={handleBackButtonClick} type='button'>
        <span className='visually-hidden'>Предыдущая фотография</span>
      </button>
      <button className='slider__button slider__button--next' disabled={isNextButtonDisabled(currentPictureIndex, pictures)} onClick={handleNextButtonClick} type='button'>
      ...
      <ul className='silder__pictures'>
        {getSlicedPictures(currentPictureIndex, pictures).map((picture) => (
          <li key={picture.id} className='silder__pictures-item'>
            <img
              className='slider__picture'
              src={picture.url.mini}
